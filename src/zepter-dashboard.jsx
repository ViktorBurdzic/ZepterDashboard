...
function Dashboard({ userId, onLogout }) {
    ...
    const api = useMemo(() => new ZepterAPI(), []);
    ...
    useEffect(() => {
        loadUserData();
        const interval = setInterval(() => loadUserData({ silent: true }), 30000); // Refresh every 30s
    ...
    const handleTransaction = async (e) => {
        ...
        try {
            ...
            setTxResult(result);
            setTxAmount('');
            await loadUserData({ silent: true }); // Refresh user data
        } catch (err) {
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    <div>
                        ...
                        <h1 style={{ margin: 0, fontSize: '24px', fontWeight: '700', letterSpacing: '-0.02em' }}>
                            Zepter Commission Engine Dashboard
                        </h1>
                        <p style={{ margin: 0, fontSize: '13px', color: '#94A3B8' }}> </p>
                    </div>
                </div>
